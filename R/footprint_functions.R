...
footprint <- function(country = "AUT", consumption = "food", nutrient = "calories", 
                      year = 2013, y, X = X, v = vers, index, 
                      result.dir = "data", result.suffix = ""){

  if(nutrient == "calories"){
    L <- readRDS(file=paste0("/mnt/nfs_fineprint/tmp/fabio/v",v,"/cal/",year,"_L.rds"))
  }

  # extract relevant final demand vector
  Y_country <- y[, Y_codes$iso3c == country & Y_codes$fd %in% consumption]
  Y_target <- rowSums(Y_country)
  ...
  # remove FP matrices from memory (they are large!)
  rm(FP); gc()
  ...
  results[,`:=`(group_origin  = index$comm_group[match_origin],
                ...
                region_origin  = index$region[match_origin],
                primary = items_group$group[match(results$item_origin, items_group$item)])]
  results[country_origin==country, region_origin := country]
  results[, value := ifelse(primary %in% c("Primary crops", "Fish"), value, 0)]
  results[, primary := NULL]
  
  # reorder columns
  results <- results %>%
    relocate(value, .after = region_origin) %>%
    relocate(c(iso_origin, region_origin), .after = country_origin) %>%
